...
static inline QVariant::Type DecodeNumber(int Prec,int Scale)
{
	if (Scale>0)
...
static QVariant::Type qDecodeOCIType(const QString& ocitype, int ocilen, int ociprec, int ociscale)
{
    ...
    if (ocitype == QLatin1String("VARCHAR2") || ocitype == QLatin1String("VARCHAR")
        ...
        type = DecodeNumber(ociprec,ociscale);
...
QVariant::Type qDecodeOCIType(int ocitype,int Prec,int Scale)
{
    QVariant::Type type = QVariant::Invalid;
    switch (ocitype) {
    case SQLT_STR:
...
{
    Q_D(const QOCIDriver);
	  if (type & 0x80) //Speciaal: ALLEEN de tabellen/synonyms/view van het huidige schema!
		{
      QStringList tl;
